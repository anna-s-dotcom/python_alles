...
# from sklearn import preprocessing
features_notNorm = features.copy()
...
# for i, val in X_test.iloc[:1, :].iterrows():
for i, val in X_test.iterrows():
    ...
    y_temp = y_train.reindex(neigh.index)
...
print('Acc Score:', metrics.accuracy_score(y_testarr, y_pred))

# Zeilen: wahren Werte, Spalten: vorhergesagten Werte
print(pd.crosstab(y_testarr, y_pred))
